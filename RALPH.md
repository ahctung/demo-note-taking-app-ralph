...
2. Picks the next logical task from the list (`tasks.json`)
...
- **`tasks.json`** - Task list with `passes: false/true`
...
grep '"passes": false' tasks.json | wc -l  # Count remaining
```
...
**Bottom line**: Define tasks in `tasks.json`. Run Ralph. Review commits. Ship.
