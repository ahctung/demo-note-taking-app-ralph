...
# Iteratively works through features in tasks.json until complete
#
...
for ((iteration=1; iteration<=MAX_ITERATIONS; iteration++)); do
  ...
  result=$(claude --dangerously-skip-permissions -p "@$TASKS_FILE @$PROGRESS_FILE @SPEC.MD @CLAUDE.md

Pick ONE task from $TASKS_FILE where passes=false. 
